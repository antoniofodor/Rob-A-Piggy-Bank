...
folder=ROOT/'assets/acorn'
raw=(folder/'acorn.glb').read_bytes()
...
binary=raw[start+8:]
assert len(doc['nodes'])==len(doc['meshes'])==3
assert {n['name'] for n in doc['nodes']}=={'Nut','Cap','Stem'}
...
assert abs(stats['Nut']['bounds']['min'][1])<1e-5
assert max(v['bounds']['max'][1] for v in stats.values())<.85
assert max(v['bounds']['max'][0]-v['bounds']['min'][0] for v in stats.values())<.65
img=doc['images'][0];view=doc['bufferViews'][img['bufferView']]
png=binary[view['byteOffset']:view['byteOffset']+view['byteLength']]
assert png==(folder/'acorn-basecolor.png').read_bytes()
