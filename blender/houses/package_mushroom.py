...
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'assets/houses/models/mushroom-v1'
...
E.indent(root);E.ElementTree(root).write(OUT/'mushroom-collision-mounts.rbxmx',encoding='utf-8',xml_declaration=True)
positions=[]
for line in (OUT/'mushroom-visual.obj').read_text().splitlines():
