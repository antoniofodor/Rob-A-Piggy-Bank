...
slug=sys.argv[sys.argv.index('--')+1];out=ROOT/'assets/houses/models'/f'{slug}-v1';r=json.loads((out/'geometry-report.json').read_text())
bpy.ops.wm.open_mainfile(filepath=str(out/f'{slug}.blend'));deps=bpy.context.evaluated_depsgraph_get()
...
bpy.ops.import_scene.fbx(filepath=str(out/f'{slug}-visual.fbx'));count=sum(o.type=='MESH' for o in bpy.context.scene.objects)
