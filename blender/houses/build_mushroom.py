...
OUT=ROOT/'assets/houses/models/mushroom-v1';OUT.mkdir(parents=True,exist_ok=True)
...
bpy.ops.wm.obj_export(filepath=str(OUT/'mushroom-visual.obj'),export_selected_objects=True,forward_axis='Z',up_axis='Y',export_materials=True)
try:
 bpy.ops.export_scene.fbx(filepath=str(OUT/'mushroom-visual.fbx'),use_selection=True,object_types={'MESH'},apply_scale_options='FBX_SCALE_UNITS',axis_forward='Z',axis_up='Y',bake_anim=False,add_leaf_bones=False)
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'mushroom.blend'))
(OUT/'geometry-report.json').write_text(json.dumps(report,indent=2))
