...
def package(slug):
 out=ROOT/'assets/houses/models'/f'{slug}-v1';r=json.loads((out/'geometry-report.json').read_text())
 ...
 E.indent(root);E.ElementTree(root).write(out/f'{slug}-collision-mounts.rbxmx',encoding='utf-8',xml_declaration=True)
 pts=[list(map(float,line.split()[1:4])) for line in (out/f'{slug}-visual.obj').read_text().splitlines() if line.startswith('v ')]
