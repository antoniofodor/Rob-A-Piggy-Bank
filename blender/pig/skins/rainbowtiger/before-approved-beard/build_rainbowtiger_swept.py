...
# Option C uses three flowing cheek locks, compact inner-ear accents and a
# single swept plume. No large mane, ankle cuffs or angry eyebrow wedges.
# Each lock follows a cubic curve; a shared UV atlas adds subtle strand shading.
...
for side in (-1,1):
    ...
    # Downward C- and S-curves replace the old sideways upper cheek fan.
    paths=[
      ((.60,-.74,.22),(.82,-.94,.03),(.77,-1.09,-.18),(.625,-1.18,-.13),.185,.090),
      ((.67,-.69,.25),(.92,-.79,-.02),(.77,-.93,-.32),(.80,-.90,-.40),.145,.076),
      ((.67,-.80,.01),(.89,-1.01,-.21),(.78,-1.13,-.37),(.595,-1.16,-.43),.185,.093)]
    for i,(a,b,c,d,w,h) in enumerate(paths):
        points=[Vector((side*x,y,z)) for x,y,z in (a,b,c,d)]
        lock('CheekFur_'+str(side),points,w,h,bone,shape=i)
...
# Stagger long single-curve and S-shaped locks down the face. Their hidden narrow
# roots overlap the wider middles above, and the lowest locks meet at the chin.
beard_paths=[
 ((.73,-.75,.16),(1.04,-.83,-.07),(.74,-.99,-.65),(.75,-.99,-.79),.155,.084),
 ((.66,-.82,-.18),(.82,-1.01,-.46),(.49,-1.08,-.62),(.56,-1.03,-.83),.177,.093),
 ((.58,-.89,-.30),(.61,-1.12,-.55),(.40,-1.17,-.70),(.49,-1.10,-.78),.175,.093),
 ((.45,-.90,-.49),(.52,-1.12,-.74),(.17,-1.16,-.76),(.21,-1.10,-.91),.172,.094)]
for side in (-1,1):
    for i,(a,b,c,d,w,h) in enumerate(beard_paths):
        points=[Vector((side*x,y,z)) for x,y,z in (a,b,c,d)]
        # Small deliberate asymmetry avoids a row of mirrored pendant shapes.
        if side>0:
            points[1].x+=.025;points[2].z+=.018*(i%3-1);points[3].z+=.025
        lock('BeardFur',points,w,h,'Root','charcoal',shape=i)
lock('BeardFur',[(-.23,-.94,-.57),(-.34,-1.14,-.73),(-.05,-1.18,-.83),(-.13,-1.11,-.96)],.166,.098,'Root','charcoal',shape=1)
lock('BeardFur',[(.25,-.94,-.57),(.34,-1.13,-.74),(.07,-1.17,-.83),(.16,-1.10,-.94)],.163,.094,'Root','charcoal',shape=3)
lock('BeardFur',[(-.04,-.98,-.61),(-.16,-1.13,-.77),(.15,-1.19,-.87),(0,-1.10,-.995)],.175,.105,'Root','charcoal',shape=2)
...
for o in meshes:
    if o.name.startswith(('Body','CheekFur','EarFur','BeardFur')):
...
report['animation']['charcoalFur']={'parts':['CheekFur_-1','CheekFur_1','EarFur','BeardFur'],'colorTexture':'rainbowtiger-charcoal-fur.png','behavior':'Preserve shared UV color map, white part tint; dark roots blend into smoky silver tips with fine subtle strand shading.'}
report['animation']['beardFur']={'part':'BeardFur','colorTexture':'rainbowtiger-charcoal-fur.png','behavior':'Reference banana-shaped locks: widest through the middle, softly tapered at both ends. Layered C-curves and S-curves flow downward from the redesigned upper cheeks to the central chin. Beard follows Root; upper cheeks retain Ruff_L/R motion.'}
report['design']+='; full inner-ear fans and downward C/S-curved facial locks, thickest at the middle and tapered at both ends'
