...
"""Build STORM WOLF as a node graph, in its own blend file.
    ...
    blender.exe --background --python skins/stormwolf/make_stormwolf_blend.py

Writes `skins/stormwolf/stormwolf.blend`. Then the ordinary loop, plus the
...
# independent on purpose: a script in `make/` and a script in `skins/stormwolf/`
