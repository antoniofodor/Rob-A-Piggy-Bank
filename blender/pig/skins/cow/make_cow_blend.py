...
"""Build the Dairy Cow's coat as a node graph, in its own blend file.
    ...
    blender.exe --background --python skins/cow/make_cow_blend.py

Writes `skins/cow/cow.blend`. Then the ordinary loop:
...
# independent on purpose: a script in `make/` and a script in `skins/cow/` are
