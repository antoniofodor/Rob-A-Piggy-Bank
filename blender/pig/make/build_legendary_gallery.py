...
assert r['closedBack'] and r['closedBackRaysChecked']==360 and not r['closedBackRayMisses']
...
shots=[('hero','Three-quarter, during a full strike'),('rear','Restored painted back and low centered tail'),('side','Original tail seated over the repaired attachment'),('crown','Continuous mane, with the coin location concealed'),('front','Original face and coat'),('vault','Close-up of the restored rear surface')]
...
body=f'<a href="{relative(OUT/"index.html",PACKAGE)}">← Legendary skins</a><p class="small">STORM WOLF · LAYOUT AND LIGHTNING REVISION</p><h1>Storm Wolf</h1><p class="note">A continuous painted back, with the original tail seated over the repaired attachment. The mane covers the coin deposit location. Original painted coat and crystal details retained, with chunky branching lightning and synchronized body glow.</p>{nav}{video}<p class="small">{r["meshCount"]} meshes · {r["totalTriangles"]:,} triangles · Studio integration pending</p><div class="grid">{figures}</div>'
(PACKAGE/'index.html').write_text(page('Storm Wolf layout revision',body),encoding='utf-8')
link=relative(PACKAGE,OUT)
body=f'<nav><a href="../common/index.html">Common animals</a><a href="../rare/index.html">Rare skins</a></nav><p class="small">ROB A PIGGY BANK · LEGENDARY SKINS</p><h1>Legendary piggies.</h1><p class="note">Storm Wolf: a restored closed back, a low centered tail, continuous mane, and chunky branching lightning and glowing body markings that pulse with each strike.</p><article><video controls autoplay muted loop playsinline poster="{link}/stormwolf-lightning-on.png" src="{link}/stormwolf-lightning.mp4"></video><h2>Storm Wolf</h2><nav><a href="{link}/index.html">Animation and six views</a><a href="{link}/stormwolf-complete.blend">Blender</a><a href="{link}/stormwolf-complete.fbx">FBX</a></nav></article><p class="small">One revised review package. Other legendary designs have not been rebuilt in this pass.</p>'
...
The Storm Wolf now has a continuous painted back. The obsolete hollow chamber and rear vault cut have been removed, restoring the original source surface and its UVs. Its original tail sits over the repaired attachment with a slight underlap and a low 5-degree lift. There is no protruding collar. The mane stays continuous over the coin deposit position, with no visible slot or rim, following the user's review. All six old blue bolt meshes are removed and replaced with eight long, chunky, jagged strikes and eight substantial tapered forks. The largest roots are 0.60 studs across. Painted cyan lightning has a separate emission mask and pulses in sync; the texture pattern stays fixed.

The painted coat and original sources remain unchanged. The retained exterior UV loops were checked against their source values with maximum error {max(u['maxUVError'] for u in r['uv']):.9f}. A tiny original mesh pinhole was closed. All 360 rays across the former opening hit the closed rear surface. Body and Tail are closed manifold meshes.
...
No runtime, economy, crate catalogue, uploads or publishing changes were made. The repaired Body mesh still needs uploading and the revised Body, Tail and tail-lightning mesh IDs and bounds replacing in Config; existing materials, glow timing and scale can stay.
