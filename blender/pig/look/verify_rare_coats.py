...
rows=[]
for key in SPECS:
    folder=Path(paths.skin_dir(key));package=Path(paths.animal_package(key))
    ...
    for group,color in zip(('body','trim'),colors):
        ...
        assert hashlib.sha256(mask.read_bytes()).digest()==hashlib.sha256((folder/mask.name).read_bytes()).digest()
