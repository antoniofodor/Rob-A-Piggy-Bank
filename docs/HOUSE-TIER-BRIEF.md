...
Every house opens into the shared room system (`HOUSE-TROPHY-ROOMS.md`,
brief B2). The mapping above assigns each new house to **cozy, classic,
modern or royal**; where a house wants its own family (gloop, hoard, sky,
gold) propose it as a *variant* of one of the four with that four as the
tested fallback, so a house never opens into nothing. Every template
carries the named mount points from B2 (`Featured`, `Shelf_N`, `Wall`,
`Record`, `Plaque_Legacy`, `Door_Exit`).
