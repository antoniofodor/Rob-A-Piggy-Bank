"""Upload the three Arcade Legendary geometry revisions; retain all v1 receipts."""
import argparse
import hashlib
import json
import time
...
KEYS = ['mechaplayer','finalboss','jackpot']
...
def inputs(key):
    folder = ROOT / 'assets/piggies/legendary' / key
    report = json.loads((folder/'package/arcade-v2/asset-report.json').read_text())
    assert report['baseGeometryAndUVPreserved']
    assert report['animation']['loopClosureError'] < .0001
    assets=[]
    for part in report['parts']:
        assert part['triangles']<20000 and part['nonManifoldEdges']==0
    for item in report['exports']:
        path=folder/'package/arcade-v2'/item['file']
        assert sha(path)==item['sha256']
        assert item['roundTripBoundsError']<.001
        if 'accessories' in path.name:assets.append(('accessories_v2',path,'Model','model/fbx'))
    if key=='mechaplayer':
        for item in report['textures']:
            path=folder/item['file'];assert sha(path)==item['sha256']
            assets.append((item['role']+'_v2',path,'Image','image/png'))
...
def main():
    ...
    if not (args.go or args.check):
        ...
        print('Validated three revised Legendary accessory models and two Mecha maps.')
        return
