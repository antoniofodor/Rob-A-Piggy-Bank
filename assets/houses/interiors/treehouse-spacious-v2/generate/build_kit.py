...
def add_model(parent, template_name, name=None, offset=None, yaw=0, opened=False):
    ...
    for source in spec["parts"]:
        ...
        if opened and group.startswith("Panel_"):
            ...
            part["position"][1] += (16.1-(2.24+row*4.45))*28/22.55
...
def add_scale_avatar(parent):
    # A six-stud block avatar, removable as one review-only Model.
    model, _ = item(parent, "Model", "ScaleAvatar_DeleteMe")
    pieces = [("Torso", (0, 4, 3.1), (2, 1, 2), "iron"),
              ("Head", (0, 4, 4.95), (1.4, 1.4, 1.7), "plasterLight"),
              ("LeftArm", (-1.5, 4, 3.1), (1, 1, 2), "plasterLight"),
              ("RightArm", (1.5, 4, 3.1), (1, 1, 2), "plasterLight"),
              ("LeftLeg", (-.5, 4, 1.05), (1, 1, 2.1), "woodDark"),
              ("RightLeg", (.5, 4, 1.05), (1, 1, 2.1), "woodDark")]
    for name, pos, size, mat in pieces:
        add_part(model, {"name": name, "position": rb(pos), "size": [size[0], size[2], size[1]],
            "rotation": IDENTITY[:], "material": mat, "collide": False,
            "shape": "Block", "transparency": 0, "group": "Reference"})
...
def build_outputs():
    ...
    spec = {"palette": PALETTE, "templates": TEMPLATES,
        "units": "studs", "forward": "-Z", "roomLength": 38,
        "clearWidth": 46, "centerLane": 20, "doorClearWidth": 32.6*46/38,
        "doorClearHeight": 13.4*28/22.55, "slotsPerRoom": 6,
    ...
    root = root_xml()
    kit, kp = item(root, "Model", "TreehouseSpaciousInteriorKit")
    ...
    root = root_xml()
    demo, _ = item(root, "Model", "TreehouseSpaciousInterior_Review")
    add_model(demo, "AchievementVestibule", "Entrance")
    for room in range(1):
        add_model(demo, "RoomShell", f"Room_{room+1:02}", [0, 0, -38*room])
        add_model(demo, "RebirthGate", f"Gate_{room+1:02}", [0, 0, -38*(room+1)], opened=False)
    ...
    add_scale_avatar(demo)
    save_xml(root, "treehouse-interior-review.rbxmx")
    enlarged = copy.deepcopy(TEMPLATES)
    TEMPLATES.clear()
    TEMPLATES.update(json.loads((OUT/"generate/original-geometry.json").read_text())["templates"])
    original = root_xml()
    old, _ = item(original, "Model", "OriginalComparison")
    add_model(old, "AchievementVestibule", "Entrance")
    add_model(old, "RoomShell", "Room_01")
    add_model(old, "RebirthGate", "Gate_01", [0, 0, -32])
    for slot in range(6):
        at = TEMPLATES["RoomShell"]["mounts"][f"Slot_{slot+1:02}"]
        add_model(old, "Pedestal", f"Pedestal_{slot+1:02}", at["position"], -90 if slot < 3 else 90)
    add_scale_avatar(old)
    save_xml(original, "original-comparison.rbxmx")
    TEMPLATES.clear()
    TEMPLATES.update(enlarged)
