...
class Asset:
 def __init__(self,slug,palette):
  self.slug=slug;self.out=ROOT/'assets/houses'/f'{house_slug(slug)}-v1';self.out.mkdir(parents=True,exist_ok=True)
  ...
  self.visual=[];self.cutaway=[];self.colliders=[];self.mounts={};self.mountYaw={};self.routes=[];self.palette=palette;self.mat={}
 ...
 def open_door(self,width,spring,floor,mat,frame):
  ...
  return o
 def mount(self,name,p,yaw=0):
  # A MOUNT CARRIES WHICH WAY ITS DISPLAY LOOKS, because the art is the only
  # thing that knows. The runtime used to aim every display at the DOORWAY,
  # derived from the Door_Exit mount -- which is right for a display on the
  # back wall and wrong for one on a side wall, where it turns the object about
  # 43 degrees off the shelf it is standing on and presents its diagonal to its
  # neighbour. `yaw` is degrees about Blender z, 0 being the frame a mount has
  # always had, so an unrotated mount behaves exactly as it did.
  self.mounts[name]=list(p);self.mountYaw[name]=yaw
  o=bpy.data.objects.new(name,None);self.scene.collection.objects.link(o);o.location=p;o.rotation_euler.z=math.radians(yaw);o.empty_display_type='ARROWS';o.empty_display_size=.45
 ...
 def finish(self,camera,interior,look,scale,meta):
  ...
  bpy.ops.export_scene.fbx(filepath=str(self.out/f'{house_slug(self.slug)}-visual.fbx'),use_selection=True,object_types={'MESH'},apply_scale_options='FBX_SCALE_UNITS',axis_forward='Z',axis_up='Y',bake_anim=False,add_leaf_bones=False)
