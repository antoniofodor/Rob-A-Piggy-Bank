# The Treehouse — Blender prototype

Raised oak cabin, switchback stairs, guarded deck, side bridge and trophy shelves.
...
- `treehouse.blend`: editable source, named meshes, trophy marker empties.
- `treehouse-visual.fbx`: static visual export, without review lights/camera/ground.
- `treehouse-visual.obj` and `.mtl`: alternate export and material colours.
- `treehouse-collision-mounts.rbxmx`: anchored invisible collision guides, Root,
...
- 113 visual mesh objects; 7,016 triangles total.
- 61 collision parts plus Root; **175 BaseParts** before trophies
  if each exported mesh becomes one MeshPart. Importer splitting can add parts.
- Complete visual bounds: **39.350 W × 36.700 D × 31.480 H**.
  Includes open door, roof, foliage and exterior decorations.
- Front projection: local Roblox Z=-11.000; rear Z=25.700.
- Floor at Y=10; doorway 5.4 wide.
- 217 route centreline samples passed the offline clearance/support check.
...
- FBX round trip preserved all 113 mesh objects and matched authored
...
Animation: None; Rare. All routes and decks are static.
...
```powershell
& 'C:/Program Files/Blender Foundation/Blender 5.2/blender.exe' -b -t 4 --python blender/houses/build_treehouse.py
python blender/houses/package_assets.py treehouse
& 'C:/Program Files/Blender Foundation/Blender 5.2/blender.exe' -b -t 4 --python blender/houses/verify_assets.py -- treehouse
