# Ladybird — complete animal asset v1
...
- `ladybird-complete.blend`: six separated meshes, baked texture materials, packed PNGs and review cameras/lights.
- `ladybird-complete.fbx`: complete pig, with body, snout, ears, legs, tail and optional eyes. Textures embedded; external originals are supplied too.
- `ladybird_body_color.png`, `ladybird_trim_color.png`: exact copies of the existing 1024×1024 coats, verified by SHA-256.
- `ladybird-hero.png`, `-front.png`, `-crown.png`, `-spine.png`: rendered from the exported geometry. Neutral studio lighting for inspection; not a live Roblox screenshot.
- `ladybird-asset-report.json`: mesh/triangle counts, source hashes, texture assignments and FBX round-trip measurements.
- `blender/pig/skins/ladybird/ladybird.blend` and `blender/pig/skins/ladybird/make_ladybird_blend.py`: original procedural coat authoring scene and generator, preserved.
...
```powershell
& 'C:/Program Files/Blender Foundation/Blender 5.2/blender.exe' -b -t 4 --python-exit-code 1 --python blender/pig/make/package_animal.py -- --skin ladybird
