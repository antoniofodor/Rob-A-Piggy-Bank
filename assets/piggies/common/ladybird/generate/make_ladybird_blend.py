...
"""Build the Ladybird's shell as a node graph, in its own blend file.
    ...
    blender.exe --background --python assets/piggies/common/ladybird/generate/make_ladybird_blend.py

Writes `assets/piggies/common/ladybird/source/ladybird.blend`. Then the ordinary loop:
...
is a disc of radius 0.105 centred on (+-0.318, -0.889, 0.364); and the snout's
nostril bowls sit at y -1.225, which is inside the black head and is the one
thing this skin knowingly costs -- see `HEAD`.

(That landmark used to be read off a `NostrilPreview` object, two thin blades
seated in the bowls. They were retired and are out of the blends as of
2026-09-21; the BOWLS are still cut into the snout by `build_pig.py` and are
what the number above describes, so nothing here moved.)
"""
...
# independent on purpose: a script in `make/` and a generator in `assets/piggies/common/ladybird/generate/`
