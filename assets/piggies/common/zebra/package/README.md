# Zebra — complete animal asset v1
...
- `zebra-complete.blend`: six separated meshes, baked texture materials, packed PNGs and review cameras/lights.
- `zebra-complete.fbx`: complete pig, with body, snout, ears, legs, tail and optional eyes. Textures embedded; external originals are supplied too.
- `zebra_body_color.png`, `zebra_trim_color.png`: exact copies of the existing 1024×1024 coats, verified by SHA-256.
- `zebra-hero.png`, `-front.png`, `-crown.png`, `-spine.png`: rendered from the exported geometry. Neutral studio lighting for inspection; not a live Roblox screenshot.
- `zebra-asset-report.json`: mesh/triangle counts, source hashes, texture assignments and FBX round-trip measurements.
- `blender/pig/skins/zebra/zebra.blend` and `blender/pig/skins/zebra/make_zebra_blend.py`: original procedural coat authoring scene and generator, preserved.
...
```powershell
& 'C:/Program Files/Blender Foundation/Blender 5.2/blender.exe' -b -t 4 --python-exit-code 1 --python blender/pig/make/package_animal.py -- --skin zebra
