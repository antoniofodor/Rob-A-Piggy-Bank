...
"""Build the Zebra's coat as a node graph, in its own blend file.
    ...
    blender.exe --background --python assets/piggies/common/zebra/generate/make_zebra_blend.py
...
while not (os.path.exists(os.path.join(_root, "paths.py"))
           or os.path.exists(os.path.join(_root, "blender", "pig", "paths.py"))):
    _up = os.path.dirname(_root)
    if _up == _root:
        raise RuntimeError("cannot find blender/pig/paths.py above %s" % __file__)
    _root = _up
if not os.path.exists(os.path.join(_root, "paths.py")):
    _root = os.path.join(_root, "blender", "pig")
