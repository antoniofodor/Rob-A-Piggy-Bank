...
bpy.ops.wm.open_mainfile(filepath=str(HOME/'package/overdrive-v3'/f'{KEY}-overdrive-v3.blend'))
scene=bpy.context.scene;scene.name='SPELLCASTER - full robe and boots v4';scene.frame_set(1)
...
for frame in (1,19,37,55,73,91,109,127,145):
 ...
 for o in extras:
  if o.name=='MageContinuousRobe':continue # face opening is checked geometrically in geometry.py
...
scene.frame_set(1)
package=HOME/'package/mage-v4';package.mkdir(exist_ok=True);preview=HOME/'preview/mage-v4';preview.mkdir(exist_ok=True)
exports=[]
for label,obs in [('accessories',extras),('complete',list(parts.values())+extras)]:
 bpy.ops.object.select_all(action='DESELECT')
 for o in obs:o.select_set(True)
 bpy.context.view_layer.objects.active=obs[0];path=package/f'{KEY}-mage-v4-{label}.fbx'
...
report=json.loads((HOME/'package/overdrive-v3/asset-report.json').read_text());report.update(revision='arcade-mage-v4',status='Review only; geometry not installed',displayName='Spellcaster',parts=checks,exports=exports,animation={'loopSeconds':6,'fps':24,'frames':[1,145],'loopClosureError':closure,'faceClearanceSampledFrames':9})
report['validationRobe']=robe_validation
...
report['design']='Mage revision: continuous purple robe beneath the belly, integrated violet mantle around the ears, full fitted boots with gold cuffs and crystal clasps. Preserves the existing shoulder gems, glowing channels, rear focus ring, aura and motion. Coin slot and face remain clear.'
...
bpy.ops.wm.save_as_mainfile(filepath=str(package/f'{KEY}-mage-v4.blend'))
