# Glacier — rare skin, first art pass

Asset ID proposal: `glacier`. Based on the approved `tiger` procedural coat.
Palette: base #EFFAFF, ink #429BCC, nose #6EB9DD, ear #AADEEF, pale #DAEFFA.

## Files

- `glacier-complete.blend`: assembled pig with packed colour and emissive masks, review lighting and camera.
- `glacier-complete.fbx`: six separated static meshes with embedded colour textures. Assign the supplied emissive masks explicitly in Studio; do not assume the FBX importer recreates Blender's emission graph.
- `glacier_body_color.png`, `glacier_trim_color.png`: opaque 1024×1024 colour sheets.
- `glacier_body_emissive.png`, `glacier_trim_emissive.png`: 1024×1024 grayscale glow masks; white marks the sparse highlights.
- Five review renders, including dim lighting, and `glacier-asset-report.json`.
- `blender/pig/skins/glacier/coat-spec.json`: palette, parent scene hash and preview glow strength.
...
In Blender, run `blender/pig/make/build_rare_coat.py -- --skin glacier`, then `make/bake_skin.py -- --skin glacier`, then `make/package_animal.py -- --skin glacier --name "Glacier"`. The durable palette definitions are in `blender/pig/rare_coats.py`; the builder reuses the parent graph without saving the parent. Run `build_rare_gallery.py` after rebuilding packages.
