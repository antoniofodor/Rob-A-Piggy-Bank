# Peppermint — rare skin, first art pass

Asset ID proposal: `peppermint`. Based on the approved `tiger` procedural coat.
Palette: base #FFF8E9, ink #D02542, nose #D23145, ear #FFB2B5, pale #FFF8E9.

## Files

- `peppermint-complete.blend`: assembled pig with packed colour and emissive masks, review lighting and camera.
- `peppermint-complete.fbx`: six separated static meshes with embedded colour textures. Assign the supplied emissive masks explicitly in Studio; do not assume the FBX importer recreates Blender's emission graph.
- `peppermint_body_color.png`, `peppermint_trim_color.png`: opaque 1024×1024 colour sheets.
- `peppermint_body_emissive.png`, `peppermint_trim_emissive.png`: 1024×1024 grayscale glow masks; white marks the sparse highlights.
- Five review renders, including dim lighting, and `peppermint-asset-report.json`.
- `blender/pig/skins/peppermint/coat-spec.json`: palette, parent scene hash and preview glow strength.
...
In Blender, run `blender/pig/make/build_rare_coat.py -- --skin peppermint`, then `make/bake_skin.py -- --skin peppermint`, then `make/package_animal.py -- --skin peppermint --name "Peppermint"`. The durable palette definitions are in `blender/pig/rare_coats.py`; the builder reuses the parent graph without saving the parent. Run `build_rare_gallery.py` after rebuilding packages.
