...
def build(key):
    with (logs/(key+'.log')).open('w',encoding='utf-8') as log:
        p=subprocess.run([r'C:\Program Files\Blender Foundation\Blender 5.2\blender.exe','--background','--python-exit-code','1','--python',str(HERE/'build_arcade.py'),'--','--skin',key],cwd=ROOT,stdout=log,stderr=subprocess.STDOUT)
