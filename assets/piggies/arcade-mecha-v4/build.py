"""Mecha review revision: open coin slot, fitted panels, paint, face plate and boots.
Preserves installed v2 assets; exports a separate v4 review package.
"""
...
bpy.ops.wm.open_mainfile(filepath=str(HOME/'package/arcade-v3'/f'{KEY}-arcade-v3.blend'))
scene=bpy.context.scene;scene.name='MECHA v4 - inset panels and fitted hardware';scene.frame_set(1)
scene['revision']='arcade-mecha-v4';scene['runtimeStatus']='Review preview, not installed'
...
for theta in range(0,360,5):
 for lat in range(54,90,4):
  ...
  projected=surface(theta,lat,.07)
  if abs(projected.x)<.13 and -.06<projected.y<.60:continue # intentional functional coin aperture
...
# The original opening and all accessory meshes must leave a vertical coin path.
for x in (-.035,0,.035):
 for y in (.07,.15,.25,.35,.45):
  origin=Vector((x,y,2));direction=Vector((0,0,-1))
  for ob in [parts['Body']]+extras:
   inv=ob.matrix_world.inverted();hit,co,*_=ob.ray_cast(inv@origin,inv.to_3x3()@direction)
   if hit:assert (ob.matrix_world@co).z<.78,(ob.name,'blocked coin slot',x,y)
# Eye centers must remain ahead of the new face plate, including the outward eye cap.
for x in (-.3181,.3181):
 hit,co,*_=bpy.data.objects['MechFacePlate'].ray_cast(Vector((x,-2,.3636)),Vector((0,1,0)))
 assert hit and co.y>parts['EyePreview'].location.y-.07,('face plate obscures eye',tuple(co))

# Parent motion remains looped; static face plate is checked above.
rest={o.name:o.matrix_world.copy() for o in extras}
for frame in (1,19,37,55,73,91,109,127,145):
 scene.frame_set(frame);bpy.context.view_layer.update()
 for o in extras:
  if o.name.startswith(('MechFacePlate','MechInsetCheekConnector')):continue # fitted face armor is intentional; eyes/snout checked separately
...
scene.frame_set(1)
package=HOME/'package/arcade-v4';package.mkdir(exist_ok=True);preview=HOME/'preview/arcade-v4';preview.mkdir(exist_ok=True)
exports=[]
for label,obs in [('accessories',extras),('complete',list(parts.values())+extras)]:
 bpy.ops.object.select_all(action='DESELECT')
 for o in obs:o.select_set(True)
 bpy.context.view_layer.objects.active=obs[0];path=package/f'{KEY}-arcade-v4-{label}.fbx'
...
report=json.loads((HOME/'package/arcade-v2/asset-report.json').read_text());report.update(revision='arcade-mecha-v4',status='Review preview; not installed in game',parts=checks,exports=exports,animation={'loopSeconds':6,'fps':24,'frames':[1,145],'loopClosureError':closure,'faceClearanceSampledFrames':9})
report['runtimeNotes']='Mecha v4 review only. Original coin slot reopened, flat skewed warning markings, recessed side/rear vents, fitted face plate and modular joint boots. V2 remains installed.'
if new_textures:report['textures'] += [{'role':'shoulder_color','file':str((ROOT/p).relative_to(HOME)), 'sha256':hashlib.sha256((ROOT/p).read_bytes()).hexdigest(),'size':[512,512]} for p in new_textures]
report['validation']={'crownCoverageRaysOutsideCoinSlot':coverage_rays,'coinSlotClearanceRays':15,'baseMeshGeometryAndUVsPreserved':True,'eyeReseatForwardBlenderUnits':.026,'insetVents':3,'navyLegSockets':4,'shoulderMarkings':'UV texture paint'}
...
for label,pos,target in [('hero',(-4,-6,2.8),(0,0,.10)),('back',(-4,6,2.8),(0,0,.10)),('top',(-2,3,7),(0,0,.15))]:
 cam.location=pos;cam.rotation_euler=(Vector(target)-cam.location).to_track_quat('-Z','Y').to_euler();scene.render.filepath=str(preview/f'{KEY}-v4-{label}.png');bpy.ops.render.render(write_still=True)
cam.location=(-4,-6,2.8);cam.rotation_euler=(Vector((0,0,.1))-cam.location).to_track_quat('-Z','Y').to_euler();scene.frame_set(37);scene.render.filepath=str(preview/f'{KEY}-v4-motion.png');bpy.ops.render.render(write_still=True);scene.frame_set(1)
bpy.context.preferences.filepaths.save_version=0
bpy.ops.wm.save_as_mainfile(filepath=str(package/f'{KEY}-arcade-v4.blend'))
print('V4_COMPLETE',KEY,'parts',len(extras),'triangles',sum(x['triangles'] for x in checks if x['role']=='accessory'),flush=True)
