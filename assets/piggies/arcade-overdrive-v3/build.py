...
scene=bpy.context.scene;scene.name='OVERDRIVE - Legendary upgrade v3';scene.frame_set(1)
...
sheet=HOME/'sheets/overdrive-v3';sheet.mkdir(exist_ok=True)
for group,objects in {'body':[parts['Body']],'trim':[parts[n] for n in ('Snout','Ears','Legs','Tail')]}.items():
 ...
 path=sheet/f'{KEY}_overdrive_v3_{group}_color.png';image.filepath_raw=str(path);image.file_format='PNG';image.save();image.pack()
...
scene.frame_set(1)
package=HOME/'package/overdrive-v3';package.mkdir(exist_ok=True);preview=HOME/'preview/overdrive-v3';preview.mkdir(exist_ok=True)
exports=[]
for label,obs in [('accessories',extras),('complete',list(parts.values())+extras)]:
 bpy.ops.object.select_all(action='DESELECT')
 for o in obs:o.select_set(True)
 bpy.context.view_layer.objects.active=obs[0];path=package/f'{KEY}-overdrive-v3-{label}.fbx'
...
report=json.loads((HOME/'package/arcade-v2/asset-report.json').read_text());report.update(revision='arcade-overdrive-v3',status='Review only; geometry not installed',displayName='Overdrive',parts=checks,exports=exports,animation={'loopSeconds':6,'fps':24,'frames':[1,145],'loopClosureError':closure,'faceClearanceSampledFrames':9})
if new_textures:report['textures']=new_textures
report['tierRules']={'tier':'legendary','geometry':'fitted shields, layered armor, mounted reactor and cartridges','glow':'pink channels, charge cells and diamond cores','motion':'hinged shields, pulsing cores and rotating reactor','aura':'existing sparse pink arcade pixels'}
...
report['name']='Overdrive'
report['design']='Overdrive: a powered-up arcade pig with a clean pink face and feet, fitted plum and violet shields, thin gold borders, routed pink glow tracks, four-cell charge meters, pulsing diamond cores, hinged shoulder motion, rotating rear reactor and sparse pink pixel aura. The original functional coin slot remains open.'
...
bpy.ops.wm.save_as_mainfile(filepath=str(package/f'{KEY}-overdrive-v3.blend'))
print('OVERDRIVE_COMPLETE',KEY,'parts',len(extras),'triangles',sum(x['triangles'] for x in checks if x['role']=='accessory'),flush=True)
