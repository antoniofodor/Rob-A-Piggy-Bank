...
scene=bpy.context.scene;scene.name='JACKPOT - jeweled shades and coin payout v3';scene.frame_set(1)
...
for frame in (1,19,37,55,73,91,109,127,145):
 ...
 for o in extras:
  if o.name.startswith('JackpotShades'):continue # user-requested eyewear intentionally covers eyes
...
scene.frame_set(1)
package=HOME/'package/jackpot-v3';package.mkdir(exist_ok=True);preview=HOME/'preview/jackpot-v3';preview.mkdir(exist_ok=True)
exports=[]
for label,obs in [('accessories',extras),('complete',list(parts.values())+extras)]:
 bpy.ops.object.select_all(action='DESELECT')
 for o in obs:o.select_set(True)
 bpy.context.view_layer.objects.active=obs[0];path=package/f'{KEY}-jackpot-v3-{label}.fbx'
...
report=json.loads((HOME/'package/arcade-v2/asset-report.json').read_text());report.update(revision='arcade-jackpot-v3',status='Review only; not installed',parts=checks,exports=exports,animation={'loopSeconds':6,'fps':24,'frames':[1,145],'loopClosureError':closure,'faceClearanceSampledFrames':9})
report['textures']=[t for t in report.get('textures',[]) if t['role'].startswith('reel_')]+new_textures
report['design']='Jeweled gold sunglasses, patterned burgundy and gold coat, twin payout chutes and twelve tumbling coins emitted from the machine. Preserves the three animated reels and lights.'
report['payout']=payout_validation
...
cam=scene.camera;cam.data.ortho_scale=4.15
for label,pos,target in [('hero',(-4,-6,2.8),(0,0,.10)),('back',(-4,6,2.8),(0,0,.10)),('top',(-2,3,7),(0,0,.30))]:
...
bpy.ops.wm.save_as_mainfile(filepath=str(package/f'{KEY}-jackpot-v3.blend'))
print('JACKPOT_V3_COMPLETE',KEY,'parts',len(extras),'triangles',sum(x['triangles'] for x in checks if x['role']=='accessory'),flush=True)
