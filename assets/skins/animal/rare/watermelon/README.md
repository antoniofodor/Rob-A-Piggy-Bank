# Watermelon — rare skin, first art pass

Asset ID proposal: `watermelon`. Based on the approved `ladybird` procedural coat.
Palette: base #56A93A, ink #24251A, nose #F2586E, ear #FF929A, pale #FB627C.

## Files

- `watermelon-complete.blend`: assembled pig with packed colour and emissive masks, review lighting and camera.
- `watermelon-complete.fbx`: six separated static meshes with embedded colour textures. Assign the supplied emissive masks explicitly in Studio; do not assume the FBX importer recreates Blender's emission graph.
- `watermelon_body_color.png`, `watermelon_trim_color.png`: opaque 1024×1024 colour sheets.
- `watermelon_body_emissive.png`, `watermelon_trim_emissive.png`: 1024×1024 grayscale glow masks; white marks the sparse highlights.
- Five review renders, including dim lighting, and `watermelon-asset-report.json`.
- `blender/pig/skins/watermelon/coat-spec.json`: palette, parent scene hash and preview glow strength.
...
In Blender, run `blender/pig/make/build_rare_coat.py -- --skin watermelon`, then `make/bake_skin.py -- --skin watermelon`, then `make/package_animal.py -- --skin watermelon --name "Watermelon"`. The durable palette definitions are in `blender/pig/rare_coats.py`; the builder reuses the parent graph without saving the parent. Run `build_rare_gallery.py` after rebuilding packages.
