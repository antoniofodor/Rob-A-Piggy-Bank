# Leopard — complete animal asset v1
...
- `leopard-complete.blend`: six separated meshes, baked texture materials, packed PNGs and review cameras/lights.
- `leopard-complete.fbx`: complete pig, with body, snout, ears, legs, tail and optional eyes. Textures embedded; external originals are supplied too.
- `leopard_body_color.png`, `leopard_trim_color.png`: exact copies of the existing 1024×1024 coats, verified by SHA-256.
- `leopard-hero.png`, `-front.png`, `-crown.png`, `-spine.png`: rendered from the exported geometry. Neutral studio lighting for inspection; not a live Roblox screenshot.
- `leopard-asset-report.json`: mesh/triangle counts, source hashes, texture assignments and FBX round-trip measurements.
- `blender/pig/skins/leopard/leopard.blend` and `blender/pig/skins/leopard/make_leopard_blend.py`: original procedural coat authoring scene and generator, preserved.
...
```powershell
& 'C:/Program Files/Blender Foundation/Blender 5.2/blender.exe' -b -t 4 --python-exit-code 1 --python blender/pig/make/package_animal.py -- --skin leopard
